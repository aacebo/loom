...
use crate::bench::{BenchDataset, BenchResult, Decision};
...
impl ScoreExport {
    ...
    pub fn from_results(
        ...
        let result_lookup: HashMap<&str, &crate::bench::SampleResult> = result
            .sample_results
            .iter()
            .map(|r| (r.id.as_str(), r))
            .collect();
        ...
        let mut category_samples: HashMap<
            String,
            Vec<(&crate::bench::BenchSample, &crate::bench::SampleResult)>,
        > = HashMap::new();
...
fn build_category_export(
    name: &str,
    samples: &[(&crate::bench::BenchSample, &crate::bench::SampleResult)],
    raw_scores: &HashMap<String, HashMap<String, f32>>,
) -> CategoryExport {

...
pub async fn exec(
    ...
    let (result, raw_scores) = if batch_size > 1 {
        bench::run_batch_async_with_scores_config(&dataset, scorer, run_config, progress_callback)
            .await
    } else {
        bench::run_async_with_scores_config(&dataset, scorer, run_config, progress_callback).await
    };
    ...
    let export = bench::ScoreExport::from_results(&dataset, &result, raw_scores);

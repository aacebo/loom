...
A comparison of how each system decides what qualifies as a memory worth storing, with Merc shown **with all proposed improvements implemented**.

> **Note:** This document represents Merc with all 8 improvements from [improvement-proposals.md](../merc/improvement-proposals.md) in place. See [memory-system-comparison.md](./memory-system-comparison.md) for current-state comparison.
...
flowchart TD
    subgraph Positive [Positive Labels: 32]
        ...
        subgraph Context [Context: 13]
            ...
            C11[Request NEW]
            C12[Update NEW]
            C13[Announcement NEW]
        end
    end

    subgraph Negative [Negative Labels: 3]
        N1[Filler NEW]
        N2[Acknowledgment NEW]
        N3[Repetition NEW]
    end

    style E8 fill:#3b82f6,color:#fff
    style O8 fill:#3b82f6,color:#fff
    style C10 fill:#3b82f6,color:#fff
    style C11 fill:#3b82f6,color:#fff
    style C12 fill:#3b82f6,color:#fff
...
## Hindsight: LLM Extraction (Unchanged)
...
MERC (Current):
  - Same scores but threshold = 0.75
  - Would still accept, but closer to boundary
```
...
MERC (Current) without Context:
  - "It's really getting to me" alone
  - Stress: 0.65 (ambiguous without context)
  - Might be REJECTED
```
...
### Hindsight's Store-Everything (Unchanged)
...
| Metric | Merc (Current) | Merc | Hindsight |
|--------|----------------|-----------------|-----------|
| **Write Latency** | ~50ms | ~100-150ms | ~500-2000ms |
| **Write Cost** | ~$0.0001 | ~$0.0002 | ~$0.001-0.01 |
| **Label Coverage** | 26 | 35 | Unlimited (LLM) |
| **Context Awareness** | None | 1 turn | Full conversation |
| **Redundancy** | None | 2-model ensemble | None |
| **Calibration** | None | Platt scaling | None |
| **Adaptive Thresholds** | No | Yes | N/A |
| **Negative Filtering** | 1 label (Phatic) | 4 labels | None |
| **Feedback/Learning** | None | Logged for tuning | None explicit |

## Expected Accuracy Gains

| Improvement | Expected Gain |
|-------------|---------------|
| Hypothesis Optimization | +5-15% |
| Confidence Calibration | +10-20% FP/FN reduction |
| Dynamic Thresholds | +5-10% precision |
| Label Expansion (+6) | +10-15% recall |
| Negative Label Boosting (+3) | -20-30% noise |
| Context Window | +10-20% on follow-ups |
| Lightweight Ensemble | +15-25% error reduction |
| Feedback Loop | Continuous improvement |

**Combined estimate:** 30-50% accuracy improvement over current Merc

## Complexity Comparison

| Operation | Merc (Current) | Merc (Improved) | Hindsight |
|-----------|----------------|-----------------|-----------|
| Write-time model calls | 1 | 2 (parallel) | 1 (LLM) |
| Write-time compute | O(1) | O(2) parallel | O(n) tokens |
| Storage per input | 0 or 1 | 0 or 1 + log | 2-5 facts |
| Filtering logic | Simple threshold | Multi-stage | None |
| Configuration params | ~30 | ~50+ | ~20 |

---

## TL;DR

| Dimension | Merc | Hindsight | Winner |
|-----------|-----------------|-----------|--------|
| **Storage efficiency** | Only stores important content | Stores everything | **Merc** |
| **Write latency** | ~100-150ms | ~500-2000ms | **Merc** |
| **Write cost** | ~$0.0002 | ~$0.001-0.01 | **Merc** |
...
| **Redundancy** | 2-model ensemble | Single LLM | **Merc** |
| **Noise reduction** | 4 negative labels | None | **Merc** |
| **Adaptability** | Dynamic thresholds | Static extraction | **Merc** |

**Merc closes the gap** on flexibility and context awareness while maintaining its advantages in efficiency, interpretability, and noise reduction.
...
- **Gap narrowed:** Merc addresses most weaknesses while staying 3-10x faster than Hindsight

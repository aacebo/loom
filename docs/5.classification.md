...
Loom's labels provide signals for type classification:
...
flowchart TD
    ...
    H -->|No| J{Score >= 0.75?}
    J -->|Yes| K[Type: Working → promote later]
    J -->|No| L[Reject]
...
Loom provides the signals needed for classification but doesn't perform classification itself:

### Loom Output for Classification

...
The Loom scoring engine evaluates input text across **four independent dimensions** using zero-shot classification, then aggregates weighted predictions to produce a final importance score.
...
For complete scoring algorithm details, see [Scoring Algorithm](./loom/scoring-algorithm.md).
...
Storage is handled by **downstream systems**—Loom is stateless and only provides scoring and classification signals.

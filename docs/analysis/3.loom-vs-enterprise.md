...
└── <a href="../README.md">▾ 6. Research/</a>
    ...
    └── <a href="./README.md">▾ 6.3 Analysis/</a>
        ...
        ├── <span><a href="./3.loom-vs-enterprise.md"><b>6.3.3 Loom vs Enterprise</b></a> 👈</span>
        └── <a href="./4.hindsight-vs-zep.md">6.3.4 Hindsight vs Zep</a>
</pre>

A comparison of Loom's approach vs. enterprise memory patterns (synthesized from public research, referred to as "Enterprise Model" for comparison).

## Core Architectural Difference

```mermaid
flowchart TD
    subgraph Loom [Loom: Write-Time Gating]
...
**Key difference:** Loom gates at write time—scoring and rejecting low-value content. Enterprise Model classifies and tags **all** content, storing everything with metadata that determines handling and retention.

---

## Classification Approaches

| Aspect | Loom | Enterprise Model |
|--------|------|--------|
...
**Key difference:** Loom produces a binary decision with score; Enterprise Model extracts structured project context.
...
**Key difference:** Loom has no PII awareness; Enterprise Model detects, tags, and applies compliance handling.
...
**Key difference:** Loom explicitly rejects phatic content; Enterprise Model stores but naturally deprioritizes via graph sparsity.
...
**Key difference:** Loom may reject valuable business intel if scores are low; Enterprise Model stores with appropriate confidentiality handling.

---

## Trade-offs

### Loom Write-Time Gating
...
| Metric | Loom | Enterprise Model | Winner |
|--------|------|--------|--------|
| **Classification latency** | ~100-150ms | ~50-100ms (BERT only) | **Enterprise Model** |
| **End-to-end latency** | ~100-150ms | ~6-15s (with extraction) | **Loom** |
| **LLM cost (classification)** | $0 | $0 | Tie |
| **LLM cost (extraction)** | $0 | ~$0.01-0.05/chunk | **Loom** |
| **Storage per message** | 0 or 1 record | 2-10 entities/facts | **Loom** |
...
- **Loom** asks: "Is this important enough to remember?" → Selective, efficient gating
...
| Dimension | Loom | Enterprise Model | Winner |
|-----------|------|--------|--------|
| **Classification speed** | ~100-150ms | ~50-100ms | **Enterprise Model** |
| **Total processing time** | ~100-150ms | ~6-15s | **Loom** |
| **Cost per message** | ~$0 | ~$0.0001 (classification only) | **Loom** |
| **Cost with extraction** | ~$0 | ~$0.01-0.05 | **Loom** |
...
| **Write-time filtering** | Yes | No | **Loom** |
| **Operational simplicity** | 1 model | BERT + LLM | **Loom** |
...
Loom and Enterprise Model can work together:

```mermaid
flowchart LR
    A[Input] --> B[Loom Scoring]
    B --> C{Score >= 0.75?}
...
- **Loom as pre-filter:** Use Loom's scoring to gate what enters Enterprise Model, reducing extraction costs
- **Loom for real-time:** Filter during conversation, batch accepted content to Enterprise Model
- **Cost optimization:** ~75% of messages rejected by Loom = ~75% LLM cost savings for Enterprise Model extraction

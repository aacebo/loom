...
Documentation for the Loom scoring engine—a write-time memory gating system using zero-shot classification.
...
Loom is a **fast write-time gating filter** for AI memory systems:
...
Loom focuses on one thing well—scoring. Other concerns are handled downstream:
...
| System | Key Insight | How Loom Applies |
|--------|-------------|------------------|

...
|----|------|----------------|---------------|--------|
| LOOM-001 | Hypothesis Optimization | 0% | 5-15% | ✅ Complete |
| LOOM-002 | Platt Calibration | <1% | 10-20% | ✅ Infrastructure Complete |
| LOOM-003 | Dynamic Thresholds | 0% | 5-10% | ✅ Complete |

**Combined potential:** 20-35% accuracy improvement with zero latency cost.

---

## LOOM-001: Hypothesis Optimization
...
cargo run -p loom-cli -- bench extract-scores \
  datasets/dataset.json \
  -o raw_scores.json

# 2. Train Platt parameters (pure Rust using tch)
cargo run -p loom-cli -- bench train \
  raw_scores.json \
  -o platt_params.json \
  --generate-rust
...
cargo run -p loom-cli -- bench run datasets/dataset.json -v
```
...
- [x] Raw score extraction CLI — *Run with `loom bench extract-scores`*
- [x] Platt training CLI — *Run with `loom bench train`*
- [ ] Latency benchmarks before/after — *Future work*
- [ ] Accuracy benchmarks before/after — *Run with `loom bench run`*

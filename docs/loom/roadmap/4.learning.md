...
|----|------|----------------|---------|--------|
| LOOM-010 | Feedback Logging | <1ms | Capture decisions for review | 🔲 Not Started |
| LOOM-011 | Benchmark Dataset | 0% | Measure accuracy | ✅ Complete |
| LOOM-012 | Weight Tuning Pipeline | 0% | Continuous improvement | 🔲 Not Started |

**Goal:** Enable data-driven weight tuning without impacting runtime performance.

---

## LOOM-010: Feedback Logging
...
```bash
# Validate dataset
cargo run --package loom-cli -- bench validate datasets/dataset.json

# Run benchmark
cargo run --package loom-cli -- bench run datasets/dataset.json

# Check label coverage
cargo run --package loom-cli -- bench coverage datasets/dataset.json
```
...
- `libs/loom-cli/` — CLI crate with bench subcommand
...
- [x] Create CLI (`loom bench run/validate/coverage`)
...
## LOOM-012: Weight Tuning Pipeline
...
|--------|-------------------|
| Loom (proposed) | Feedback logs → weight tuning |
...
Loom's approach is simpler—tune static weights based on observed errors—but achieves a similar goal of continuous improvement.

...
|----|------|----------------|---------------|
| LOOM-008 | Context Window | +50% | 10-20% on follow-ups |
| LOOM-009 | Lightweight Ensemble | +100% | 15-25% error reduction |

**Combined potential:** 25-45% accuracy improvement at the cost of ~100-150ms total latency.

---

## LOOM-008: Context Window
...
Loom's approach balances context awareness with latency. One previous turn provides significant improvement with minimal cost.
...
|--------|---------------------|
| Loom (proposed) | 2-model ensemble |
| Zep | Single LLM (no redundancy) |
| Hindsight | Single LLM (no redundancy) |
| Enterprise Model | Single BERT + single LLM |

Loom's ensemble approach is unique among these systems. The cost is latency, but the benefit is significantly reduced errors on ambiguous inputs.

...
| Approach | Pros | Cons |
|----------|------|------|
| **Write-time filtering (Loom)** | Low storage cost, fast retrieval | May miss context-dependent value |
| **Read-time filtering** | Preserves everything | High storage cost, slower retrieval |

Loom chooses write-time filtering because:
...
**Current limitation:** Loom is stateless and doesn't consider conversation history. See [Phase 3: Context](./loom/roadmap/3.context.md) for planned improvements.
...
See [Scoring Algorithm](./loom/scoring-algorithm.md) for full configuration options.
